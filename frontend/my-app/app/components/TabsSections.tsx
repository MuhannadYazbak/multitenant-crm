...
export default function TabsSection({ tenant, entityType, entityId }: TabsSectionProps) {
    ...
    // 2. Single Unified useEffect Hook
    useEffect(() => {
    ...
    const handleAddNote = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setNoteContent("");
            setIsPinned(false);
            await fetchTabData(); // Await full re-fetch!
        } catch (err) {
                    ...
                    <>
                        ...
                        {activeTab === "notes" && (
                            <div className="space-y-6">
                                ...
                                <div  id='noteslist' className="space-y-3">
                                    {items.map((note) => (
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="border rounded-lg overflow-hidden text-xs">
                                    <div className="p-2.5 bg-gray-50 border-b flex justify-between items-center">
                                        ...
                                        <button id='archive/unarchive' onClick={() => setShowArchivedDocs(!showArchivedDocs)} className="text-blue-600 hover:underline font-medium">
                                        ...
                                        <tbody className="divide-y">
                                            {items.map((entry) => (
                                                <tr key={entry.id} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="p-2 text-right">
                                                        <button onClick={() => handleDeleteBilling(entry.id)} className="text-red-500 hover:bg-red-50 font-bold">🗑️</button>
                                                    </td>
                                                </tr>
                                            ))}
                                            {items.length === 0 && (

...
Prerequisites
Ensure the backend virtual environment contains all required dependencies.

Ensure PostgreSQL is running locally with the target database created (saas_mvp_test).
...
🔑 Environment Variables ConfigurationThe testing suite relies on environment variables passed into the webServer orchestrator inside playwright.config.ts:DATABASE_URL: postgresql://postgres:***@localhost:5432/saas_mvp_test (Enforces test isolation)NEXT_PUBLIC_API_BASE_URL: http://localhost:8000 (Configures frontend backend target)SECRET_KEY: HMAC-SHA256 compliant key ($\ge$ 32 characters)
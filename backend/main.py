...
def get_tenant_info(tenant_name: str, db: Session = Depends(get_db)):
...
def update_client(
    ...
    # Capture exact values in local variables BEFORE db.commit()
    res_id = db_client.id
    res_name = db_client.name
    res_phone = db_client.phone
    res_email = db_client.email
